...
# --- API Key Setup ---
try:
    # Use st.secrets for deployment, fallback to environment variable for local dev
    OPENAI_API_KEY = st.secrets.get("OPENAI_API_KEY") or os.getenv("OPENAI_API_KEY")
    if not OPENAI_API_KEY:
        st.error("OpenAI API key not found. Please set it in Streamlit secrets or as an environment variable.", icon="🚨")
        st.stop()
    client = openai.OpenAI(api_key=OPENAI_API_KEY)
except (TypeError, AttributeError):
    st.error("OpenAI API key configuration is incorrect. Please check your setup.", icon="🚨")
    st.stop()
